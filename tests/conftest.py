...
from release_flow_with_sample_api import DefaultAioHttpClient, ReleaseFlowWithSampleAPI, AsyncReleaseFlowWithSampleAPI
from release_flow_with_sample_api._utils import is_dict
...
logging.getLogger("release_flow_with_sample_api").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[ReleaseFlowWithSampleAPI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with ReleaseFlowWithSampleAPI(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncReleaseFlowWithSampleAPI]:
